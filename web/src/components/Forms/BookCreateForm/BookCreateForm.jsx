...
const BookCreateForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (s3CoverUrl.result === "success") {
      ...
      try {
        const book = await handleFetch(`${process.env.REACT_APP_API_ADDRESS}/book`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json"
          },
        ...
        if (book.result !== "success") throw new Error();

        window.alert("Resenha cadastrada");

        return navigate("/perfil");
      } catch (e) { window.alert("Não foi possível cadastrar o livro."); }
  ...
  return (
    <form className="p-4 bg-white lg:border lg:border-gray-200 lg:rounded-lg lg:shadow-2xl lg:p-8">
      ...
      />
      <ButtonPrimary type="submit" text="Criar resenha" onClick={handleSubmit} className="mr-2" />
    </form>
  );
}
